...
import type { BoardTask, BoardColumn } from '@/lib/types';

interface KanbanViewProps {
  columns: BoardColumn[];
  setColumns: (fn: (prev: BoardColumn[]) => BoardColumn[]) => void;
  slug: string;
  projectKey: string;
  onRefresh: () => void;
}

export default function KanbanView({ columns, setColumns, slug, projectKey, onRefresh }: KanbanViewProps) {
  ...
  const handleDragOver = (event: DragOverEvent) => {
    ...
    if (!destCol) destCol = columns.find((c) => c.id === overId);
  ...
  const handleQuickAdd = async (columnId: string) => {
    ...
    const res = await fetch(`/api/workspaces/${slug}/tasks`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ title: quickAddTitle.trim(), columnId }),
    });
    setCreating(false);
    if (res.ok) {
      setQuickAddTitle('');
